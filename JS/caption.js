...
function preprocess(imgElement) {
    ...
    return tf.tidy(() => {
        let tensor = tf.fromPixels(imgElement).toFloat();  // convert image pixel to array
        const resized = tf.image.resizeBilinear(tensor,[224,224]);  //resized the image in 224 ,224
        const offset = 127.5;   // normalization
        const normalized = resized.div(offset).sub(tf.scalar(1.0)); 
        const batched = normalized.expandDims(0); //expand dim
        return batched;
    });
}
...
async function start() {    
    ...
    const mobilenet = await tf.loadModel('img_model_mobilenet/model.json');  //load the training model  (CNN mobile net)
    const layer = mobilenet.getLayer('conv_preds');  //taking output
    console.log("mobileNet loaded"); // model loaded
    mobileNet =  tf.model({         //model construct
        'inputs': mobilenet.inputs,   // take image as a input
        'outputs': layer.output        // give the feature of image
    });

    model = await tf.loadModel('lang_model_lstmbidi/model.json');  // load Lstm model(caption generating model)
...
function modelLoaded() {
    ...
    isModelLoaded = true;
    text.innerHTML = "Good to go!";  // Both models are loaded
}


imageLoader.addEventListener('change', function(e) {   //take image and show the image in frontend
...
function playSentence(text) {    
      let msg = new SpeechSynthesisUtterance(text);   // speech generator
